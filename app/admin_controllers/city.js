    ...
    exports.genetare_city_excel = function (req, res) {
        if (typeof req.session.userid != 'undefined') {
            ...
            City.aggregate([lookup, unwind, search, filter, sort]).then((array) => {
                ...
                sheet1.set(1, 1, req.__('title_country'));
                sheet1.set(2, 1, req.__('title_city'));
                sheet1.set(3, 1, req.__('title_city_code'));
                sheet1.set(4, 1, req.__('title_bussiness'));
                ...
                array.forEach(function (data, index) {
                    ...
                    if (data.isBusiness == 1) {
                        sheet1.set(4, index + 2, req.__('title_on'));
                    } else {
                        sheet1.set(4, index + 2, req.__('title_off'));
                    }
                    ...
                    if (index == array.length - 1) {
                        workbook.save(function (err) {
                            if (err)
                            {
                                ...
                                var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_city.xlsx"
                                res.json(url);
                                setTimeout(function () {
    ...
    exports.add_city_form = function (req, res) {
        ...
        if (typeof req.session.userid != "undefined") {
        	...
        	var zone_types = utils.SURCHARGE_ZONE_TYPES();
            Country.find({}).then((country) => { 
                ...
                res.render('add_city_form', {country_data: country, payment_gateway: types, map_url: url, zone_types: zone_types});
                            ...
                            CityZone.find({'cityid': array[0]._id}).then((city_zone) => { 
                                RedZoneArea.find({'cityid': array[0]._id}).then((red_zone_area) => { 
                                    ...
                                    console.log(red_zone_area)
                                        res.render('add_city_form', {detail: array, red_zone_area: red_zone_area, airport: airport, map_url: url, city_zone: city_zone,  moment: moment, city_list: city_list, payment_gateway: types, dest_city_list: dest_city_list, zone_types: zone_types});
...
exports.update_city_detail = function (req, res) {
    ...
    req.body.city_business = req.body.city_business || 0;
    req.body.zone_business = req.body.zone_business || 0;
	req.body.surcharge_rules = req.body.surcharge_rules || 0;
