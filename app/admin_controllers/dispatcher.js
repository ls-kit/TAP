    ...
    if (typeof req.session.dispatcher == 'undefined') {
        ...
        async.concat([Trip, Trip_history], function (model, callback) {
            ...
            function (err, result) {
                ...
                if (result.length == 0) {
                    res.render('dispatcher_request_list', { detail: result, 'current_page': 1, 'pages': 0, 'next': 1, 'pre': 0, moment: moment });
                } else {
                    var pages = Math.ceil(total / number_of_rec);
                    res.render('dispatcher_request_list', { detail: result, 'current_page': page, 'pages': pages, 'next': next, 'pre': pre, moment: moment });
                }
            })
...
exports.dispatcher_new_request = function (req, res, next) {
        ...
        var condition = {
            $match: {
                $and: [
                    {user_type_id: Schema(req.body.dispatcher_id)}, 
        ...
        if (index == array.length - 1) {
            workbook.save(function (err) {
                if (err)
                {
                    ...
                    var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_dispatcher.xlsx"
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_dispatcher_request.xlsx"
                            res.json(url);
                            setTimeout(function () {
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_dispatcher_future_request.xlsx"
                            res.json(url);
                            setTimeout(function () {
