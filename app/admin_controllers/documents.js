...
exports.generate_document_excel = function (req, res, next) {
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        Document.aggregate([lookup, unwind, search, filter, sort]).then((array) => { 
            ...
            sheet1.set(3, 1, req.__('title_country'));
            sheet1.set(4, 1, req.__('title_type'));
            sheet1.set(5, 1, req.__('title_option'));
            ...
            array.forEach(function (data, index) {
                ...
                if (data.type == 0) {
                    sheet1.set(4, index + 2, req.__('title_user'));
                } else if (data.type == 1) {
                    sheet1.set(4, index + 2, req.__('title_provider'));
                } else if (data.type == 2) {
                    sheet1.set(4, index + 2, req.__('title_provider_vehicle'));
                }

                if (data.option == 1) {
                    sheet1.set(5, index + 2, req.__('title_mandatory'));
                } else if (data.option == 0) {
                    sheet1.set(5, index + 2, req.__('title_optional'));
                }
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_document.xlsx"
                            res.json(url);
                            setTimeout(function () {
    ...
    Document.findOne({}, function(error, document_count){
        ...
        Country.findOne({_id: Document1.countryid}).then((country_data) => {
            if (country_data) {
                ...
                } else if (req.body.type == 2) { // for vehicle documents
                    Provider.find({country: countryname}).then((providers) => {
...
exports.update_document_detail = function (req, res) {
    ...
    Document.findByIdAndUpdate(id, req.body).then((document) => {
        ...
        User_Document.update({'document_id': id}, update_data, {multi: true}).then((user_documents) => { 

            Provider_Document.update({'document_id': id}, update_data, {multi: true}).then((provider_documents) => { 

                Provider_Vehicle_Document.update({'document_id': id}, update_data, {multi: true}).then((provider_vehicle_documents) => {
