...
function generate_excel(req, res, array, timezone) {
    ...
    sheet1.set(6, 1, req.__('title_completed'));
    sheet1.set(7, 1, req.__('title_country'));
    sheet1.set(8, 1, req.__('title_registered_date'));
        ...
        if (index == array.length - 1) {
            workbook.save(function (err) {
                if (err)
                {
                    ...
                    var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_hotel.xlsx"
    ...
    if (typeof req.session.hotel != "undefined") {
        ...
        Hotel.findById(req.session.hotel._id).then((response) => {
            ...
            Country.findById(response.countryid).then((country_data) => {
                ...
                res.render("hotel_profile", {is_public_demo: is_public_demo, detail: response, country_code: lookup.alpha3,phone_number_length:country_data.phone_number_length,min_phone_number_length:country_data.phone_number_min_length});
                delete message;
            });
        });
    } else {
...
exports.hotel_create_trip = function (req, res) {
    ...
    if (typeof req.session.hotel != 'undefined') {
        ...
        Country.findOne({countryname: req.session.hotel.country}).then((country_data) => {
            ...
            res.render("hotel_create_trip", {'moment': moment, server_date: server_date, scheduled_request_pre_start_minute: setting_detail.scheduled_request_pre_start_minute, country_code: country_data.countrycode, map_key: setting_detail.web_app_google_key, hotels: req.session.hotel, country: req.session.hotel.country, phone_number_min_length: country_data.phone_number_min_length, phone_number_length: country_data.phone_number_length});
            delete message;
        });
...
exports.hotel_request = function (req, res) {
    ...
    if (typeof req.session.hotel == 'undefined') {
        ...
        var total = 0
        Trip_history.aggregate([condition, lookup, unwind, lookup1, search, filter, count]).then((Trip_history) => {
            total = Trip_history[0].total
        })

        async.concat([Trip, Trip_history], function (model, callback) {
            var query = model.aggregate([condition, lookup, unwind, lookup1, search, filter, sort, skip, limit])
            ...
            function (err, result) {
                ...
                if (result.length == 0) {
                    res.render('hotel_request_list', { detail: result, 'current_page': 1, 'pages': 0, 'next': 1, 'pre': 0, moment: moment });
                } else {
                    var pages = Math.ceil(total / number_of_rec);
                    res.render('hotel_request_list', { detail: result, 'current_page': page, 'pages': pages, 'next': next, 'pre': pre, moment: moment });
                }
            })
        // Trip_history.aggregate([filter, condition, lookup, unwind, lookup1, search, count]).then((array) => {

        //     if (array.length == 0) {
        //         res.render('hotel_request_list', {detail: array, 'current_page': 1, 'pages': 0, 'next': 1, 'pre': 0, moment: moment});
        //     } else {
        //         var pages = Math.ceil(array[0].total / number_of_rec);
        //         Trip_history.aggregate([filter, condition, lookup, unwind, lookup1, search, sort, skip, limit]).then((array) => {
...
exports.genetare_hotel_request_excel = function (req, res) {
    ...
    if (typeof req.session.hotel == 'undefined') {
        ...
        Trip_history.aggregate([filter, condition, lookup, unwind, lookup1, search, sort]).then((array) => { 
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_hotel_request.xlsx"
                            res.json(url);
                            setTimeout(function () {
    ...
    if (typeof req.session.hotel == 'undefined') {
        ...
        Trip.aggregate([filter, condition, condition1, condition2, condition3, condition4, condition5, hotel_type_condition, lookup, unwind, search, sort]).then((array) => {
            ...
            array.forEach(function (data, index) {
                ...
                if (data.payment_mode == 1) {
                    sheet1.set(8, index + 2, req.__('title_pay_by_cash'));
                } else {
                    sheet1.set(8, index + 2, req.__('title_pay_by_card'));
                }
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_hotel_schedule_trip.xlsx"
                            res.json(url);
                            setTimeout(function () {
