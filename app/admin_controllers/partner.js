...
exports.check_provider = function (req, res, next) {
    ...
    Provider.findOne({email: ((req.body.email).trim()).toLowerCase()}).then((provider) => { 
        ...
        if (provider) {
            res.json({success: false, message: req.__(admin_messages.error_message_mobile_no_already_used)})
        } else {
    ...
    if (typeof req.session.partner != 'undefined') {
        ...
        Trip_history.aggregate([filter, condition, lookup, unwind, lookup1, unwind1, search, count]).then((array) => { 
            if (array.length == 0) {
                ...
                Trip_historyTrip_history.aggregate([filter, condition, lookup, unwind, lookup1, unwind1, search, sort, skip, limit]).then((array) => { 
    ...
    if (typeof req.session.partner == "undefined") {
        Partner.findOne({email: req.body.email}).then((response) => { 
            if (response) {
                ...
                mail_notification(response.email, req.__('reset_password, link'), '');
...
exports.genetare_partner_excel = function (req, res, next) {
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        Partner.find({$and: [{$or: [query1, query2, query3, query4, query5, query6]}, query]}).then((array) => { 
            ...
            sheet1.set(5, 1, req.__('title_country'));
                ...
                if (j == array.length) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_partner.xlsx"
                            res.json(url);
                            setTimeout(function () {
                ...
                Provider.find({provider_type_id: id}).then((pro_data) => { 
                    Provider.update({provider_type_id: id}, {is_partner_approved_by_admin: change}, {multi: true}).then((provider_data) => { 
                        ...
                        if (is_approved == 0) {
                            ...
                            pro_data.forEach(function (provider) {
                                if (provider.is_approved == 1) {
                                    utils.sendPushNotification(constant_json.PROVIDER_UNIQUE_NUMBER, provider.device_type, provider.device_token, admin_messages.PUSH_CODE_FOR_PROVIDER_APPROVED, admin_messages.PUSH_NOTIFICATION_SOUND_FILE_IN_IOS);
                                }
                            });
                            ...
                            pro_data.forEach(function (provider) {
                                if (provider.is_approved == 1) {
                                    ...
                                    utils.sendPushNotification(constant_json.PROVIDER_UNIQUE_NUMBER, provider.device_type, provider.device_token, admin_messages.PUSH_CODE_FOR_PROVIDER_DECLINED, admin_messages.PUSH_NOTIFICATION_SOUND_FILE_IN_IOS);
                                }
                                provider.save().then(() => { 
                                }, (err) => {
                                        console.log(err);
...
exports.partner_requests = function (req, res, next) {
    ...
    if (typeof req.session.partner != 'undefined') {
        ...
        Trip_history.aggregate([condition, lookup, unwind, lookup1, search, filter, count]).then((array) => { 
            if (!array || array.length == 0) {
                array = [];
                res.render('partner_request_list', {detail: array, 'current_page': 1, 'pages': 0, 'next': 1, 'pre': 0, moment: moment});
            } else {
                var pages = Math.ceil(array[0].total / number_of_rec);
                Trip_history.aggregate([condition, lookup, unwind, lookup1, search, filter, sort, skip, limit]).then((array) => { 
            ...
            Partner.findOne({_id:req.session.partner._id}).then((partner)=>{
                ...
                if (req.files.length > 0)
                {
                    ...
                    partner_document.save().then(() => {
                        ...
                        ////////////////////
                        Partner_Vehicle_Document.find({_id:req.body.id,is_uploaded:0}).then((partner_pending_document) => {
                ...
                if (j == array.length) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_partner_provider.xlsx"
                            res.json(url);
                            setTimeout(function () {
        ...
        if (index == array.length - 1) {
            workbook.save(function (err) {
                if (err)
                {
                    ...
                    var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_partner_providers.xlsx"
                    res.json(url);
                    setTimeout(function () {
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_partner_request.xlsx"
                            res.json(url);
                            setTimeout(function () {
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_partner_provider_history.xlsx"
                            res.json(url);
                            setTimeout(function () {
