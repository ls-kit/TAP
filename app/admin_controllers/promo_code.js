...
exports.generate_promo_code_excel = function (req, res) {
    if (typeof req.session.userid != 'undefined') {
        ...
        Promo_Code.aggregate([lookup_country_detail, unwind_country_detail, search, filter, sort]).then((array) => { 
            ...
            sheet1.set(1, 1, req.__('title_promo'));
            sheet1.set(2, 1, req.__('title_country'));
            sheet1.set(3, 1, req.__('title_type'));
            sheet1.set(4, 1, req.__('title_value'));
            sheet1.set(5, 1, req.__('title_uses'));
            sheet1.set(6, 1, req.__('title_user_promo_used'));
            sheet1.set(7, 1, req.__('title_state'));
            ...
            array.forEach(function (data, index) {
                ...
                if (data.code_type == 2) {
                    sheet1.set(3, index + 2, req.__('title_absolute'));
                } else {
                    sheet1.set(3, index + 2, req.__('title_percentage'));
                }
                sheet1.set(4, index + 2, data.code_value);
                sheet1.set(5, index + 2, data.code_uses);
                sheet1.set(6, index + 2, data.user_used_promo);
                if (data.state == 1) {
                    sheet1.set(7, index + 2, req.__('title_active'));
                } else {
                    sheet1.set(7, index + 2, req.__('title_inactive'));
                }
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_promo_code.xlsx"
                            res.json(url);
                            setTimeout(function () {
...
exports.promo_used_info = function (req, res, next) {
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        Promo_Code.aggregate([condition, lookup_country_detail, unwind_country_detail]).then((promo_detail) => { 
            ...
            User_promo_use.aggregate([condition, lookup, unwind, lookup1, unwind1]).then((user_used_promo_array_trip) => {
                User_promo_use.aggregate([condition, lookup, unwind, lookup2, unwind2]).then((user_used_promo_array_history) => {
