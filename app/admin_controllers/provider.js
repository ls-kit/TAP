...
function generate_excel(req, res, array, timezone) {
    ...
    sheet1.set(1, 1, req.__('title_id'));
    sheet1.set(2, 1, req.__('title_name'));
    sheet1.set(3, 1, req.__('title_email'));
    sheet1.set(4, 1, req.__('title_phone'));
    sheet1.set(5, 1, req.__('title_total_request'));
    sheet1.set(6, 1, req.__('title_completed_request'));
    sheet1.set(7, 1, req.__('title_cancelled_request'));
    sheet1.set(8, 1, req.__('title_accepted_request'));
    sheet1.set(9, 1, req.__('title_city'));
    sheet1.set(10, 1, req.__('title_car'));
    sheet1.set(11, 1, req.__('title_app_version'));
    sheet1.set(12, 1, req.__('title_registered_date'));
        ...
        if (index == array.length - 1) {
            workbook.save(function (err) {
                if (err)
                {
                    ...
                    var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_provider.xlsx"
                    res.json(url);
                    setTimeout(function () {
        ...
        Provider_Document.findById(req.body.id).then((provider_document) => { 
            Provider.findOne({_id: provider_document.provider_id}).then((provider_detail) => {
                ...
                if (req.files.length > 0)
                {
                	...
                	provider_document.is_approved = 0;
                    provider_document.save(function (err) {
                        req.url = '/provider_documents';
...
exports.add_provider_vehicle = function (req, res) {
    if (typeof req.session.userid != 'undefined' || typeof req.session.partner != 'undefined') {
        ...
        Provider.findOne({_id: req.body.provider_id}, function (err, provider) {
        	...
        	var sort_condition = { $sort : { 'type_detail.service_level' : -1 } }

            Citytype.aggregate([cityid_condition, lookup, unwind, sort_condition], function (err, type_available) {
                var available_type_list = [];
            	type_available.forEach((type) => {
                    var isAcceptableType = false;
                    var isSelected = false;

                    var available_type_object = {
                        id: type._id,
                        name: type.type_detail.typename,
                        valid: isAcceptableType,
                        selected: isSelected
                    };
                    available_type_list.push(available_type_object);
                });

                res.render('edit_vehicle_detail', {type_available: type_available, vehicle_accesibility: vehicle_accesibility, type: req.body.type, provider_id: req.body.provider_id, available_type_list: available_type_list});
            });
        });
    } else {
        res.redirect('/admin');
         ...
         Provider.findOne({_id: req.body.provider_id}, function (err, provider) {
            Citytype.findOne({_id: req.body.service_type}, function (err, citytype) {
                ...
                var vehicel_json = {
                	...
                	selected_service_types: req.body.selected_service_types.map(type_id => mongoose.Types.ObjectId(type_id))
                }                
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        Provider.findOne({_id: req.body.provider_id}).then((provider) => { 
            ...
            Provider_Vehicle_Document.find({provider_id: req.body.provider_id, vehicle_id: req.body.vehicle_id}).then((provider_vehicle_document) => { 
                ...
                Citytype.aggregate([cityid_condition, lookup, unwind, sort_condition]).then((type_available) => {
                    var available_type_list = [];
                    var selectedServiceList = provider.vehicle_detail[index].selected_service_types;
                    var selectedServiceLevel = -1;
                    type_available.forEach((type) => {
                        var isAcceptableType = true;
                        if(String(type._id) !== String(provider.vehicle_detail[index].service_type)) {
                            if(type.type_detail.service_level >= selectedServiceLevel) {
                                isAcceptableType = false;
                            }
                        } else {
                            selectedServiceLevel = type.type_detail.service_level;
                        }

                        var isSelected = false;
                        if(selectedServiceList) {
                            isSelected = (selectedServiceList.find((typeID) => String(typeID) === String(type._id)) != null);
                        }
                    ...
                    res.render('edit_vehicle_detail', {provider_id: req.body.provider_id, type: 'admin', type_available: type_available, vehicle_accesibility: vehicle_accesibility, 
                        provider_vehicle_document: provider_vehicle_document, vehicle_detail: provider.vehicle_detail[index], available_type_list: available_type_list});   
                });       
            }, (err) => {
                utils.error_response(err, res);
            });
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        Provider.findOne({_id: req.body.provider_id}).then((provider) => { 
            ...
            Citytype.findOne({_id: req.body.service_type}).then((citytype) => { 
            	...
            	provider.vehicle_detail[index].selected_service_types = req.body.selected_service_types.map(type_id => mongoose.Types.ObjectId(type_id));
...
exports.generate_provider_history_excel = function (req, res, next) {
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        Trip_history.aggregate([condition, trip_condition, lookup, unwind, lookup1, search, filter, sort]).then((array) => { 
            ...
            sheet1.set(1, 1, req.__('title_id'));
            sheet1.set(2, 1, req.__('title_user_id'));
            sheet1.set(3, 1, req.__('title_user'));
            sheet1.set(4, 1, req.__('title_provider_id'));
            sheet1.set(5, 1, req.__('title_provider'));
            sheet1.set(6, 1, req.__('title_date'));
            sheet1.set(7, 1, req.__('title_status'));
            sheet1.set(8, 1, req.__('title_amount'));
            sheet1.set(9, 1, req.__('title_payment'));
            sheet1.set(10, 1, req.__('title_payment_status'));
            ...
            array.forEach(function (data, index) {
                ...
                if (data.is_trip_cancelled == 1) {
                    if (data.is_trip_cancelled_by_provider == 1) {
                        sheet1.set(7, index + 2, req.__('title_total_cancelled_by_provider'));

                    } else if (data.is_trip_cancelled_by_user == 1) {
                        sheet1.set(7, index + 2, req.__('title_total_cancelled_by_user'));
                    } else {
                        sheet1.set(7, index + 2, req.__('title_total_cancelled'));

                    }
                } else {

                    if (data.is_provider_status == 2) {
                        sheet1.set(7, index + 2, req.__('title_trip_status_coming'));

                    } else if (data.is_provider_status == 4) {
                        sheet1.set(7, index + 2, req.__('title_trip_status_arrived'));

                    } else if (data.is_provider_status == 6) {
                        sheet1.set(7, index + 2, req.__('title_trip_status_started'));

                    } else if (data.is_provider_status == 9) {
                        sheet1.set(7, index + 2, req.__('title_trip_status_completed'));

                    } else if (data.is_provider_status == 1 || data.is_provider_status == 0) {
                        if (data.is_provider_accepted == 1) {
                            sheet1.set(7, index + 2, req.__('title_trip_status_accepted'));
                        } else {
                            sheet1.set(7, index + 2, req.__('title_trip_status_waiting'));
                ...
                if (data.payment_mode == 1) {
                    sheet1.set(9, index + 2, req.__('title_pay_by_cash'));
                } else {
                    sheet1.set(9, index + 2, req.__('title_pay_by_card'));
                }

                if (data.is_pending_payments == 1) {

                    sheet1.set(10, index + 2, req.__('title_pending'));
                } else {

                    if (data.is_paid == 1) {
                        sheet1.set(10, index + 2, req.__('title_paid'));
                    } else {
                        sheet1.set(10, index + 2, req.__('title_not_paid'));
                    }
                }
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_provider_history.xlsx"
                            res.json(url);
                            setTimeout(function () {
...
exports.available_service_types_ajax = function (req, res) {
	let type_id = req.body.type_id;

    // find same and less level service types
    Citytype.findOne({'_id': type_id}).then((citytype) => {
        let category_id = citytype.typeid;

        Type.findOne({'_id': category_id}).then((category) => {
            let selected_level = category.service_level;
            let selected_type_name = category.typename;

            var city_condition = {$match: {'cityid': {$eq: citytype.cityid}}};
            var lookup = {
                $lookup:
                        {
                            from: "types",
            ...
            var unwind = {$unwind: "$type_detail"};
            // var level_condition = {$match: {$or: [{'type_detail.service_level': {$lt: selected_level}}, {'type_detail.typename': selected_type_name}]}};
            var sort_condition = { $sort : { 'type_detail.service_level' : -1 } }
            
            Citytype.aggregate([city_condition, lookup, unwind, sort_condition], function (err, type_available) {
                var available_type_list = [];
                type_available.forEach((type) => {
                    var isAcceptableType = true;
                    if(type.type_detail.typename != selected_type_name) {
                        if(type.type_detail.service_level >= selected_level) {
                            isAcceptableType = false;
                        }
                    } 

                    var isSelected = ((type.type_detail.typename == selected_type_name) || (type.type_detail.service_level < selected_level));
                ...
                res.json({success: true, types: available_type_list});                
            });
        });
    });
}

exports.provider_documents_approve = function (req, res, next) {
    if (typeof req.session.userid != 'undefined') {
        Provider_Document.findById(req.body.id).then((provider_document) => {   
            provider_document.is_approved = req.body.status;
            provider_document.save(function (err) {
                req.url = '/provider_documents';
                req.body = {id: provider_document.provider_id}
                exports.documents(req, res, next)
...
exports.provider_vehicle_documents_approve = function (req, res, next) {
    if (typeof req.session.userid != 'undefined') {
        Provider_Vehicle_Document.findById(req.body.id).then((provider_vehicle_document) => {   
            provider_vehicle_document.is_approved = req.body.status;
            provider_vehicle_document.save(function (err) {
                req.url = '/vehicle_document_list';
                req.body = {provider_id: provider_vehicle_document.provider_id, vehicle_id: provider_vehicle_document.vehicle_id}
                exports.vehicle_document_list(req, res, next)
            });
        });
    } else {
        res.redirect('/admin');
