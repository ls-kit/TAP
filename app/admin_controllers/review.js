...
exports.generate_review_excel = function (req, res, next) {
    if (typeof req.session.userid != 'undefined') {
        ...
        Reviews.aggregate([lookup, unwind, lookup1, unwind1, search, filter, sort]).then((array) => { 
            ...
            sheet1.set(2, 1, req.__('title_date'));
            sheet1.set(3, 1, req.__('title_user_email'));
            sheet1.set(4, 1, req.__('title_user_rate'));
            sheet1.set(5, 1, req.__('title_provider_email'));
            sheet1.set(6, 1, req.__('title_provider_rate'));
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_review.xlsx"
                            res.json(url);
                            setTimeout(function () {
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        var condition = {"$match": {$or: [query1, query2]}};
            Trip_history.aggregate([condition, lookup, unwind, lookup1, unwind1, search, filter, sort]).then((array) => { 
                ...
                sheet1.set(7, 1, req.__('title_cancel_by'));
                sheet1.set(8, 1, req.__('title_cancellation_reason'));
                    ...
                    if (index == array.length - 1) {
                        workbook.save(function (err) {
                            if (err)
                            {
                                ...
                                var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_cancellation_reason.xlsx"
                                res.json(url);
                                setTimeout(function () {
