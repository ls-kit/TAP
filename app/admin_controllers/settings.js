...
exports.update_app_name = function (req, res, next) {
    ...
    var fs = require('fs')
    fs.readFile('constants.json', 'utf8', function (err, data) {
        if (err) {
            ...
            fs.writeFile('constants.json', result, 'utf8', function (err) {
                if (err) {
                    return //console.log(err);
                } else {
...
exports.update_toll_guru_key = function (req, res, next) {
    if (typeof req.session.userid != 'undefined') {
        Settings.findOne({}, function (err, setting) {
            if (setting)
            {
                setting.toll_guru_key = (req.body.toll_guru_key).trim();
                setting.save();
                res.redirect('installation_settings');
            }
        });
