...
exports.generate_transaction_history_excel = function (req, res, next) {
    if (typeof req.session.userid != 'undefined') {
        ...
        Transfer_History.aggregate([lookup1, search, sort]).then((array) => { 
            ...
            var workbook = excelbuilder.createWorkbook('./data/xlsheet', time + '_transfer_history.xlsx');
            var sheet1 = workbook.createSheet('sheet1', 10, array.length + 1);


            sheet1.set(1, 1, req.__('title_id'));
            sheet1.set(2, 1, req.__('title_promo_type'));
            sheet1.set(3, 1, req.__('title_date'));
            sheet1.set(4, 1, req.__('title_email'));
            sheet1.set(5, 1, req.__('title_currency'));
            sheet1.set(6, 1, req.__('title_amount'));
            sheet1.set(7, 1, req.__('title_status'));
            sheet1.set(8, 1, req.__('title_transfer_by'));

            array.forEach(function (data, index) {
                sheet1.set(1, index + 2, data.unique_id);
                if (data.user_type == 10) {
                    sheet1.set(2, index + 2, req.__('title_user'));
                } else if (data.user_type == 11) {
                    sheet1.set(2, index + 2, req.__('title_provider'));
                }
                ...
                sheet1.set(5, index + 2, data.currency_code);
                sheet1.set(6, index + 2, data.amount);
                sheet1.set(7, index + 2, data.transfer_status);
                sheet1.set(8, index + 2, "Admin");
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_transfer_history.xlsx"
                            res.json(url);
                            setTimeout(function () {
                                fs.unlink('data/xlsheet/' + time + '_transfer_history.xlsx', function (err, file) {
                                });
                            }, 10000)
                        }
                    });
