...
function generate_excel(req, res, array, timezone) {
    ...
    sheet1.set(8, 1, req.__('title_city'));
    sheet1.set(9, 1, req.__('title_referral_code'));
    sheet1.set(10, 1, req.__('title_wallet'));
        ...
        if (index == array.length - 1) {
            workbook.save(function (err) {
                if (err)
                {
                    ...
                    var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_user.xlsx"
                    console.log(url);
                    res.json(url);
        ...
        User.find(query, function (err, users) {
            if (err) {
                ...
                Country.findOne({"countryname": users[0].country}, function (err, country_detail) {
                    City.find({"countryname": users[0].country, isBusiness: 1}, function (error, city_list) {
