...
exports.generate_wallet_history_excel = function (req, res, next) {
    if (typeof req.session.userid != 'undefined') {
        ...
        Wallet_history.aggregate([lookup, lookup1, lookup2, search, sort]).then((array) => { 
            ...
            sheet1.set(2, 1, req.__('title_type'));
            sheet1.set(3, 1, req.__('title_date'));
            sheet1.set(4, 1, req.__('title_email'));
            sheet1.set(5, 1, req.__('title_currency'));
            sheet1.set(6, 1, req.__('title_wallet_amount'));
            sheet1.set(7, 1, req.__('title_add_cut'));
            sheet1.set(8, 1, req.__('title_wallet'));
            sheet1.set(9, 1, req.__('title_wallet_description'));
            ...
            array.forEach(function (data, index) {
                ...
                if (data.user_type == 10) {
                    ...
                    sheet1.set(2, index + 2, req.__('title_partner'));
                }
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err)
                        {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_wallet_history.xlsx"
                            res.json(url);
                            setTimeout(function () {
