    ...
    Country.find({}).then((country_list) => { 
        if (typeof req.session.userid != 'undefined') {
            ...
            Trip_history.aggregate([trip_condition, trip_filter, country_filter, city_filter, trip_group_condition, lookup, search, count]).then((array) => { 
                ...
                if (array.length == 0) {
                    array = [];
                    res.render('weekly_earning', {detail: array, 'current_page': 1, provider_weekly_analytic: provider_weekly_analytic_data, country_list: country_list, city_list: city_list, type: req.body.type, 'pages': 0, 'next': 1, 'pre': 0, moment: moment , week_start_date_view : week_start_date_view , week_end_date_view : week_end_date_view});
...
exports.weekly_earning_old = function (req, res, next) {
    ...
    var trip_filter = {"$match": {}};
    trip_filter["$match"]['provider_trip_end_time'] = {$gte: start_date, $lt: end_date};
    ...
    Country.find({}, function (error, country_list) {
        if (typeof req.session.userid != 'undefined') {
            ...
            Country.findOne({_id: mongoose.Types.ObjectId(selected_country)}, function (error, country) {
                if (country)
                {
                ...
                City.findOne({_id: selected_city}, function (error, city) {
                    if (city)
                    {
                    ...
                    Trip_history.aggregate([trip_condition, trip_filter, country_filter, city_filter, trip_group_condition, lookup, search, count], function (error, array) {
                        ...
                        if (error || array.length == 0) {
                            ...
                            Trip_history.aggregate([trip_condition, trip_filter, country_filter, city_filter, trip_group_condition, lookup, search, sort, skip, limit], function (error, array) {
                                ...
                                if (error || array.length == 0) {
                                    ...
                                    Trip_history.aggregate([trip_condition, trip_filter, country_filter, city_filter, trip_group_condition_total], function (error, trip_total) {

                                        if (error || trip_total.length == 0) {
                                            array = [];
                                            res.render('weekly_earning', {detail: array, 'current_page': 1, provider_weekly_analytic: provider_weekly_analytic_data, country_list: country_list, city_list: city_list, type: req.body.type, 'pages': 0, 'next': 1, 'pre': 0, moment: moment});
                                        } else {
                                            res.render('weekly_earning', {detail: array, 'current_page': page, provider_weekly_analytic: provider_weekly_analytic_data, trip_total: trip_total, country_list: country_list, city_list: city_list, type: req.body.type, 'pages': pages, 'next': next, 'pre': pre, moment: moment});
...
exports.generate_weekly_earning_excel = function (req, res, next) {
    var array = [];
    var i = 0;
    if (req.body.page == undefined)
    {
                            ...
                            Trip_history.aggregate([trip_condition, trip_filter, country_filter, city_filter, trip_group_condition, lookup, search, sort]).then((array) => { 
                                ...
                                if (array.length == 0) {
                                    ...
                                    Trip_history.aggregate([trip_condition, trip_filter, country_filter, city_filter, trip_group_condition_total]).then((trip_total) => { 
                                        ...
                                        var workbook = excelbuilder.createWorkbook('./data/xlsheet', time + '_weekly_earning.xlsx');

                                        var sheet1 = workbook.createSheet('sheet1', 8, array.length + 1);

                                        sheet1.set(1, 1, req.__('title_provider_id'));
                                        sheet1.set(2, 1, req.__('title_name'));
                                        sheet1.set(3, 1, req.__('title_phone'));
                                        sheet1.set(4, 1, req.__('title_total'));
                                        sheet1.set(5, 1, req.__('title_cash'));
                                        sheet1.set(6, 1, req.__('title_provider_profit'));
                                        sheet1.set(7, 1, req.__('title_pay_to_provider'));
                                            ...
                                            if (index == array.length - 1) {
                                                workbook.save(function (err) {
                                                    if (err)
                                                    {
                                                        ...
                                                        var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_weekly_earning.xlsx"
                                                        res.json(url);
                                                        setTimeout(function () {
                                                            fs.unlink('data/xlsheet/' + time + '_weekly_earning.xlsx', function (err, file) {
                                                            });
                                                        }, 10000)
                                                    }
                                                });
...
exports.statement_provider_weekly_earning = function (req, res, next) {
...
exports.statement_provider_weekly_earning_old = function (req, res, next) {
            var array = [];
            var page = req.path.split('/');
            var query = {};
            query['_id'] = req.body.id;
            var provider_daily_analytic_data = {};
            var week_start_date_view = req.body.week_start_date_view;
            var week_end_date_view = req.body.week_end_date_view;

            Provider.findOne(query, function (err, provider) {

                var provider_match_condition = {$match: {'provider_id': {$eq: provider._id}}};
                var provider_daily_analytic_data = [];
                var start_date = new Date(week_start_date_view)
                var end_date = new Date(week_end_date_view)
                ...
                Provider_daily_analytic.aggregate([provider_match_condition, provider_daily_analytic_query, group_analytic_data_condition], function (error, provider_daily_analytic) {
                    ...
                    if (week_start_date_view == '' || week_start_date_view == undefined) {
                    ...
                    Trip_history.aggregate([provider_condition, trip_condition, filter, trip_group_condition], function (error, array) {
