...
exports.instant_payout_to_provider = function (req, res) {
                            ...
                            if (city) {
                                ...
                                Country.findOne({_id: country_id}).then((country) => {
                                    if (country) {
                                        ...
                                        var trip_condition_completed = {'is_trip_completed': 1};
                                        var trip_condition_cancelled = {$and: [{'is_trip_cancelled_by_user': 1}, {'pay_to_provider': {$gt: 0}}]};

                                        Trip_history.aggregate([{$match: {'confirmed_provider': {$eq: provider._id}}},
                                            {$match: {$or: [trip_condition_completed, trip_condition_cancelled]}},
                                            {$match: {'is_provider_earning_set_in_wallet': {$eq: false}}},
                                            {$match: {'is_transfered': {$eq: false}}},
                                            {$group: {_id: null, total: {$sum: '$pay_to_provider'}}}
                                        ]).then((trip) => {
                                            if (trip.length > 0) {
                                                var amount = trip[0].total;
                                                if(amount <= 0.5) {
                                                    res.json({success: false, error_code: error_message.ERROR_CODE_INVALID_EARNING_AMOUNT});
                                                } else {
                                                	amount = (amount - 0.5).toFixed(2);  // subtract transaction fee
                                                    utils.stripe_auto_transfer(amount, provider.account_id, currencycode, function (response_data) {
                                                        if (response_data.success) {
                                                            utils.add_transfered_history(Number(constant_json.PROVIDER_UNIQUE_NUMBER), provider._id, country_id,
                                                                amount, currencycode, 1, response_data.transfer_id, Number(constant_json.PROVIDER_UNIQUE_NUMBER), null);
                                                            Trip_history.update({
                                                                is_trip_completed: 1,
                                                            ...
                                                            Trip_history.update({
                                                                ...
                                                                pay_to_provider: {$gt: 0},
                                                                is_provider_earning_set_in_wallet: false,
                                                                is_transfered: false,
                                                                confirmed_provider: provider._id
                                                            }, {is_transfered: true}, {multi: true}, function (err, trip_data) {
                                                            });
                                                            provider.last_transferred_date = new Date();
                                                            provider.save();

                                                            utils.stripe_get_available_balance(provider.account_id, currencycode, function (response_data) {
                                                                if(response_data.success) {
                                                                    const payout_amount = response_data.available;
                                                                    utils.stripe_instant_payout(payout_amount, provider.account_id, currencycode, function (response_data) {
                                                                        if(response_data.success) {
                                                                            res.json({
                                                                                success: true, 
                                                                            ...
                                                                            res.json({
                                                                                success: false, 
                                                                                error_code: error_message.ERROR_CODE_INVALID_BANK_ACCOUNT_FOR_INSTANT_PAYOUT,
                                                                                stripe_error: response_data.error
                                                                            });
                                                                        }
                                                                    });
                                                                } else {
                                                                    res.json({
                                                                        success: false, 
                                                                        error_code: error_message.ERROR_CODE_GET_BALANCE_FAILED,
                                                                        stripe_error: response_data.error
                                                                    });
                                                                } 
                                                            });
                                                            ...
                                                            utils.add_transfered_history(Number(constant_json.PROVIDER_UNIQUE_NUMBER), provider._id, country_id,
                                                                amount, currencycode, 0, '', Number(constant_json.PROVIDER_UNIQUE_NUMBER), response_data.error);

                                                            res.json({success: false, error_code: error_message.ERROR_CODE_INSUFFICIENT_AMOUNT_FOR_TRANSFER});
                                                        }
                                                    })
                                                } 
                                            } else {
                                ...
                                res.json({success: false, error_code: error_message.ERROR_CODE_CITY_NOT_FOUND});
                            }
                        });
                    }
                }
...
exports.get_platform_balance = function (req, res) {
    utils.stripe_get_platform_balance(function (response_data) {
        res.json(response_data);
    });
}

exports.get_total_provider_pending_amount = function (req, res) {
    utils.check_request_params(req.body, [{name: 'provider_id', type: 'string'}], function (response) {
        if (response.success) {
            Provider.findOne({_id: req.body.provider_id}).then((provider) => {
                if (provider) {
