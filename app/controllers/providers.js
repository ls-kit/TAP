...
var CarCategories = require('mongoose').model('car_categories');
                                            ...
                                            Country.findOne({countryphonecode: provider.country_phone_code}).then((country) => {
                                                ...
                                                if(provider.is_trip.length>0){
                                                    Trip.findOne({_id: provider.is_trip[0]}).then((trip_detail)=>{
                                                        Trip_history.findOne({_id: provider.is_trip[0]}).then((trip_history_detail)=>{
                                                            if(!trip_detail){
                                ...
                                if (provider.providerLocation[0] == undefined || provider.providerLocation[1] == undefined || provider.providerLocation[0] == 0 || provider.providerLocation[1] == 0) {
                                    ...
                                    TripLocation.findOne({tripID: trip_id}).then((tripLocation) => {
                                        ...
                                        if (locations.length > 0) {
                                            ...
                                            trip.providerLocation = providerLocation;
                                            trip.total_distance = Number(total_distance.toFixed(2));
                                            Trip.findByIdAndUpdate(trip._id, {total_time: minutes, total_distance: trip.total_distance, providerLocation: trip.providerLocation, providerPreviousLocation: trip.providerPreviousLocation}, (trip_detail)=>{
                                ...
                                Citytype.aggregate([cityid_condition, lookup, unwind, sort_condition]).then((type_available) => {
                                    ...
                                    type_available.forEach((type) => {
                                        ...
                                        if(String(type._id) !== String(provider.vehicle_detail[index].service_type)) {
                                        	...
                                        	selectedCategoryName = type.type_detail.typename;
                                        }
                                    ...
                                    Type.findOne({_id: provider.vehicle_detail[index].admin_type_id}).then((type) => {
                                        if (type) {
                                            ...
                                            res.json({
                                                ...
                                                service_type_list: available_type_list,
...
exports.provider_update_vehicle_detail = function (req, res, next) {
    ...
    utils.check_request_params(req.body, [{name: 'provider_id', type: 'string'}, {name: 'vehicle_id', type: 'string'},
        // {name: 'vehicle_name', type: 'string'},{name: 'plate_no', type: 'string'},
        // {name: 'model', type: 'string'},{name: 'color', type: 'string'},
        // {name: 'passing_year', type: 'string'}, {name: 'selected_service_types', type: 'array'} 
        ], function (response) {
        if (response.success) {
            Provider.findOne({_id: req.body.provider_id}).then((provider) => {
                if (provider) {
                    if (req.body.token != null && provider.token != req.body.token) {
                        ...
                        if (index == -1) {
                            ...
                            if(req.body.selected_service_types) {
                    ...
                    if (req.body.token != null && provider.token != req.body.token) {
                        ...
                        Country.findOne({countryname: provider.country}).then((country) => {
                            ...
                            Document.find({countryid: country._id, type: 2}).then((document) => {
                            	...
                            	get_vehicle_category_from_db(req.body.provider_id, req.body.vehicle_name, req.body.model, req.body.passing_year).then((check_result) => {
                                    if(check_result.success) {
                                        // apply auto determined category and service types to vehicle
                                        vehicel_json.service_type = ObjectId(check_result.category);                                        
                                        vehicel_json.selected_service_types = check_result.types.map(type_id => ObjectId(type_id));
                                    }
...
exports.get_provider_setting_detail = function (req, res) {
    ...
    var privacy_policy_url = "https://identitymedia.s3.amazonaws.com/privacy.pdf";
...
exports.auto_select_service_levels_for_vehicle = function (req, res, next) {

    utils.check_request_params(req.body, [{name: 'provider_id', type: 'string'}, {name: 'vehicle_name', type: 'string'},
        {name: 'passing_year', type: 'string'},{name: 'model', type: 'string'}], function (response) {
        if (response.success) {
            get_vehicle_category_from_db(req.body.provider_id, req.body.vehicle_name, req.body.model, req.body.passing_year).then(result => {
                res.json(result);
            })
        } else {
            res.json({
...
function get_vehicle_category_from_db(provider_id, vehicle_name, model, passing_year) {
    return new Promise((resolve, reject) => {
        Provider.findOne({_id: provider_id}).then((provider) => {
            if (provider) {
                CarCategories.findOne({title: vehicle_name}).then((category) => {
                    if (category) {
                        const vehicle_model = category.list.find(({ model }) => model === model);
                        if(vehicle_model) {
                            let nYearMatchedIndex = -1;
                            const yearsCount = vehicle_model.list.length;
                            const param_year = passing_year;
                            for(var i = 0; i < yearsCount; i++) {
                                if((param_year >= vehicle_model.list[i].year) && ((i == yearsCount - 1) || (param_year < vehicle_model.list[i + 1].year))) {
                                    nYearMatchedIndex = i;
                                    break;
                                }
                            }
                            if(nYearMatchedIndex == -1) {
                                resolve({success: false, message: 'passing year not found'});
                            } else {
                                const vehicle_service_types = vehicle_model.list[nYearMatchedIndex].types;
                                let nMaxLevel = -1, service_category_id = null, arrSelectedServiceTypes = [];
                                if(vehicle_service_types.length > 0) {
                                    var lookup = {
                                        $lookup:
                                                {
                                                    from: "types",
                                    ...
                                    var unwind = {$unwind: "$type_detail"};
                                    var cityid_condition = {$match: {'cityid': {$eq: mongoose.Types.ObjectId(provider.cityid)}}};
                                    var sort_condition = { $sort : { 'type_detail.service_level' : -1 } }
                        
                                    Citytype.aggregate([cityid_condition, lookup, unwind, sort_condition], function (err, type_available) {
                                        type_available.forEach((type) => {
                                            const service_type = type.type_detail;                                    
                                            const typename = service_type.typename;
                                            if(vehicle_service_types.includes(typename)) {
                                                arrSelectedServiceTypes.push(type._id);
                                                if(nMaxLevel < service_type.service_level) {
                                                    nMaxLevel = service_type.service_level;
                                                    service_category_id = type._id;
                                                }
                                            }
                                        });
        
                                        if(nMaxLevel == -1) {
                                            resolve({success: false, message: 'service types not found'});
                                        } else {
                                            resolve({success: true, category: service_category_id, types: arrSelectedServiceTypes});
                                        }
                                    }); 
                                } else {
                                    resolve({success: false, message: 'service types not found'});
                            ...
                            resolve({success: false, message: 'vehicle model not found'});
                        }
                    } else {
                        resolve({success: false, message: 'vehicle name not found'});
                    }
                });
            } else {
