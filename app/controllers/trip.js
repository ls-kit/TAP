...
exports.create_trip = function (user_data, trip_type, service_type_id, req_data, response) {
    ...
    if (user_data.is_approved == 0) {
        ...
        if ((!tripData.user_type_id && user_data.wallet < 0) || (tripData.user_type_id && user_data.corporate_wallet_limit < 0)) {
...
exports.nearest_provider_list = function (citytype, req_data, response) {
    ...
    City.findOne({_id: cityid}).then((city_detail) => {
        ...
        if (city_detail) {
            ...
            provider_query["vehicle_detail.selected_service_types"] = citytype._id;
        }
...
exports.nearest_provider = function (trip, provider_id, user_favourite_providers, response) {
    ...
    City.findOne({_id: trip.city_id}).then((city_detail) => {
        ...
        if (city_detail) {
            ...
            provider_query["vehicle_detail.selected_service_types"] = trip.service_type_id;
            provider_query["is_trip"] = [];
            provider_query["is_active"] = 1;
            provider_query["is_available"] = 1;
            ...
            query.exec(function (err, providers) {
                ...
                if (providers.length == 0) {
                    ...
                    console.log(final_providers.length)
                                                            ...
                                                            City.findOne({_id: trip.city_id}).then((city) => {
                                                                ...
                                                                utils.getCurrencyConvertRate(1, countryCurrencyCode, admin_currencycode, function (response) {
                                                                    ...
                                                                    if(trip.payment_mode == constant_json.PAYMENT_MODE_APPLE_PAY){
                                                                        ...
                                                                        Card.findOne({user_id: user._id, is_default: true}, function(error, card_detail){
                                                                            ...
                                                                            if(card_detail){
                                                                                ...
                                                                                stripe.paymentIntents.create({
                                                                                    amount: Math.round((trip.total * 100)),
                                                                                    currency:  countryCurrencyCode, // user.wallet_currency_code,
                                    ...
                                    User.findOne({_id: trip.user_id}).then((user) => {
                                        ...
                                        TripLocation.findOne({tripID: req.body.trip_id}).then((tripLocation) => {
                                            ...
                                            tripLocation.save().then(() => {
                                                ...
                                                const encoded_polyline = utils.get_encoded_polyline(start_end_Location);
                                                utils.get_toll_cost(encoded_polyline, function(toll_response) {
                                                    var toll_cost = 0;
                                                    if(toll_response.success) {
                                                        toll_cost = toll_response.toll_cost;
                                                    }
                                                            ...
                                                            Provider.findOne({_id: req.body.provider_id}).then((provider) => {
                                                                City.findOne({_id: trip.city_id}).then((city) => {
                                                                    ...
                                                                    Trip_Service.findOne({_id: trip.trip_service_city_type_id}).then((tripservice) => {
                                                                        ...
                                                                        CityZone.find({cityid: trip.city_id}).then((cityzones) => {
                                                                            const surcharge_amount_result = utils.get_surcharge_amount(start_end_Location, cityzones);
                                                                            const surcharge_amount = surcharge_amount_result[0];
                                                                            ...
                                                                            if (trip.is_fixed_fare && trip.fixed_price > 0) {
                                                                                ...
                                                                                if (total_service_fees < min_fare) {
                                                                                    total_service_fees = min_fare;
                                                                                    is_min_fare_used = 1;
                                                                                }
                                                                                ...
                                                                                trip.user_tax_fee = user_tax_fee;
                                                                                total_after_tax_fees = total_service_fees;                                                                                                                                                                
                                                                            ...
                                                                            total_after_tax_fees = total_after_tax_fees + tax_fee;
                                                                            trip.total_after_tax_fees = total_after_tax_fees;
                                                                            ...
                                                                            utils.getCurrencyConvertRate(1, countryCurrencyCode, adminCurrencyCode, function (response) {
                                                                                ...
                                                                                Promo_Code.findOne({_id: trip.promo_id}).then((promocode) => {
                                                                                    ...
                                                                                    provider_service_fees = provider_service_fees - provider_miscellaneous_fee - provider_tax_fee;
                                                                                    ...
                                                                                    total = total_after_promo_payment + tax_fee;
        
                                                                                    total = total + user_miscellaneous_fee + user_tax_fee + surcharge_amount + toll_amount;
                                                                                    ...
                                                                                    trip.surcharge_amount = surcharge_amount;
                                                                                    trip.total = total;
                ...
                if (trip) {
                    City.findOne({_id: trip.city_id}).then((city) => {
                        ...
                        if (res == null) {
                            ...
                            tip_amount = Number(tip_amount.toFixed(2));
                                ...
                                Corporate.findOne({_id: trip.user_type_id}).then((corporate) => {
                                    ...
                                    Trip_Service.findOne({_id: trip.trip_service_city_type_id}).then((tripservice) => {
                                        ...
                                        if (payment_mode == constant_json.PAYMENT_MODE_CASH) {
                                            ...
                                            console.log("remaining_payment: "+remaining_payment)
                                            ...
                                            if (remaining_payment > 0) {
                                                ...
                                                // trip.save().then(() => {
                                                    Card.findOne({user_id: user_id, is_default: true}, function(error, card_detail){
                                                        ...
                                                        if(card_detail){
                                                            ...
                                                            var stripe = require("stripe")(stripe_secret_key);
                                                            console.log("wallet_currency_code: "+user.wallet_currency_code)
                                                            stripe.paymentIntents.create({
                                                                amount: Math.round((remaining_payment * 100)),
                                                                currency: trip.currencycode, // user.wallet_currency_code,
...
exports.pay_tip_payment = function (req, res, next) {
    Trip.findOne({_id: req.body.trip_id}).then((trip) => {
            ...
            if (trip) {
                ...
                stripe.paymentIntents.retrieve(trip.tip_payment_intent_id, function(error, intent){
                    ...
                    if(intent && intent.charges && intent.charges.data && intent.charges.data.length>0) {
                        ...
                        trip.total_after_surge_fees = trip.total_after_surge_fees + trip.tip_amount;
...
exports.pay_stripe_intent_payment = function (req, res, next) {
    ...
    Trip.findOne({_id: req.body.trip_id, $or: [{payment_status: PAYMENT_STATUS.WAITING }, {payment_status: PAYMENT_STATUS.FAILED }]}).then((trip) => {
        if (trip) {
            ...
            var stripe = require("stripe")(stripe_secret_key);
            stripe.paymentIntents.retrieve(trip.payment_intent_id, function(error, intent){
                console.log(intent)
                if(intent && intent.charges && intent.charges.data && intent.charges.data.length>0) {
                    utils.update_request_status_socket(trip._id);
                    trip.payment_status = PAYMENT_STATUS.COMPLETED;
...
exports.fail_stripe_intent_payment = function (req, res, next) {
        ...
        if (response.success) {
            User.findOne({_id: req.body.user_id}).then((user) => {
                if (user.token != req.body.token) {
                    ...
                    var condition1 = {$match: {$or: [{$and: [{is_trip_cancelled: {$eq: 0}}, {is_trip_end: {$eq: 1}}]}, {is_trip_cancelled_by_user: {$eq: 1}}, {is_trip_cancelled_by_provider: {$eq: 1}}]}};
        ...
        if (response.success) {
            Provider.findOne({_id: req.body.provider_id}).then((provider) => {
                if (provider.token != req.body.token) {
                    ...
                    Trip.aggregate([condition, condition1, lookup1, unwind1, filter, group, sort, skip, limit]).then((array) => {
                        Trip_history.aggregate([condition, condition1, lookup1, unwind1, filter, group, sort, skip, limit]).then((array_list) => {
                            array_list = array_list.concat(array)
                            function compare( a, b ) {
                                return new Date(b.created_at) - new Date(a.created_at);
                            }
                    ...
                    if (req.body.token != null && provider.token != req.body.token) {
                        ...
                        Trip.findOne({_id: req.body.trip_id, is_trip_end: 1}).then((trip) => {
                            Trip_history.findOne({_id: req.body.trip_id, is_trip_end: 1}).then((trip_history) => {
                                ...
                                if (trip) {
                                    if (trip.payment_status !== PAYMENT_STATUS.WAITING) {
                                        User.findOne({_id: trip.user_id}).then((user) => {
                            ...
                            Trip_history.findOne({_id: req.body.trip_id, is_trip_end: 1, payment_status: PAYMENT_STATUS.COMPLETED}).then((trip_history) => {
                                ...
                                if (trip) {
                                    ...
                                    trip.save(function(){
                                        ...
                                        if(trip.is_provider_invoice_show == 1){
                                            Trip.findOneAndRemove({_id: req.body.trip_id}).then((deleted_trip) => {
                                                var trip_history_data = new Trip_history(JSON.parse(JSON.stringify(deleted_trip)));
