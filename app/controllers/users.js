                ...
                if (!citytype) {
                    ...
                    City.findOne({_id: city_id}).then((city) => {
                        ...
                        if (!city) {
                            ...
                            utils.get_polyline_path_from_google_map(req.body.pickup_latitude, req.body.pickup_longitude, 
                                req.body.destination_latitude, req.body.destination_longitude, function(route_response) {
                        
                                if (!route_response.success) {
                                    res.json({success: false, error_code: error_message.ERROR_CODE_GOOGLE_MAP_GET_ROUTE_FAILED});
                                } else {
                                    var polyline = route_response.polyline;
                        
                                    utils.get_toll_cost(polyline, function(toll_response) {
                                        if (!toll_response.success) {
                                            res.json({success: false, error_code: error_message.ERROR_CODE_TOLL_DATA_GET_FAILED});
                                        } else {
                                            req.body.toll_cost = toll_response.toll_cost;
                                        
                                        	if(city.surcharge_rules == 1) {
                                                const path_coordinates_array = utils.get_decoded_polyline(polyline);

                                                CityZone.find({cityid: city_id}).then((cityzones) => {
                                                    const surcharge_amount_result = utils.get_surcharge_amount(path_coordinates_array, cityzones);
                                                    req.body.surcharge_amount = surcharge_amount_result[0];
                                                        ...
                                                        cityzone.forEach(function (cityzoneDetail) {
                                                            ...
                                                            if (k == zone_count) {
                                                                ...
                                                                ZoneValue.findOne({service_type_id: req.body.service_type_id,
                                                                    ...
                                                                    if (zonevalue) {
                                                                        const toll_cost = req.body.toll_cost? req.body.toll_cost: 0;                                                        
                                                                        var estimated_fare = (zonevalue.amount + toll_cost).toFixed(2);
                        ...
                        if(airport_id){
                                AirportCity.findOne({
                                    ...
                                    if (airportcity && airportcity.price > 0) {
                                        const toll_cost = body.toll_cost? body.toll_cost: 0;
                                        var estimated_fare = (airportcity.price + toll_cost).toFixed(2);
                        ...
                        City.findById(citytocity_detail.destination_city_id).then((city_detail) => {
                            if (flag == 0) {
                                ...
                                if (citytocity_detail.price > 0 && ((!city_detail.is_use_city_boundary && city_radius > destination_city_radius) || (city_detail.is_use_city_boundary && inside_city))) {
                                    const toll_cost = body.toll_cost? body.toll_cost: 0;
                                    var estimated_fare = (citytocity_detail.price + toll_cost).toFixed(2);
...
function other(cityid, citytype, body, timeMinutes, distanceKmMile, res) {
    City.findOne({_id: cityid}).then((city) => {
    	...
    	const toll_cost = body.toll_cost? body.toll_cost: 0;   
        const surcharge_amount = body.surcharge_amount? body.surcharge_amount: 0;  
        total += toll_cost + surcharge_amount;
...
exports.get_toll_cost_estimation = function(req, res) {
    utils.get_polyline_path_from_google_map(req.body.pickup_latitude, req.body.pickup_longitude, 
        req.body.destination_latitude, req.body.destination_longitude, function(route_response) {
        ...
        if (!route_response.success) {
            ...
            const polyline = route_response.polyline;

            utils.get_toll_cost(polyline, function(toll_response) {
                if (!toll_response.success) {
                    res.json({success: false, error_code: error_message.ERROR_CODE_TOLL_DATA_GET_FAILED});
                } else {
                    res.json({success: true, toll_cost: toll_response.toll_cost});
                }                
            })
        }
