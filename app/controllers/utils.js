...
exports.saveImageAndGetURL = function (imageID, req, res, id) {
    ...
    if(setting_detail.is_use_aws_bucket) {
        ...
        fs.writeFile(urlSavePicture, pictureData, 'base64', function (err) {
            // fs.unlink(imageID);
        });
    }
...
exports.sendPushNotification = function (app_type, device_type, device_token, messageCode, soundFileName, extraParam) {
    ...
    var message = {
        ...
        notification: {
            body_loc_key: 'PUSH_MESSAGE_'+messageCode,
            sound: 'default'   //FOR IOS
        },
        sound: 'enabled' //FOR ANDROID
    };

    if(/*(messageCode).toString() == (push_messages.PUSH_CODE_FOR_NEW_TRIP).toString() ||*/ messageCode == (push_messages.PUSH_CODE_FOR_PROVIDER_LOGIN_IN_OTHER_DEVICE).toString()){
    ...
    if (device_type == constant_json.PUSH_DEVICE_TYPE_IOS) {
        if (device_token == "" || device_token == null) {
            ...
            console.log("IOS PUSH NOTIFICATION TESTING");
...
var SURCHARGE_ZONE_TYPES = [
    {id: 0, name: "None"},
    {id: 1, name: "Airport"},
    {id: 2, name: "Congestion"},    
    {id: 3, name: "Out-of-town"},
    {id: 4, name: "Borough"}
];
...
exports.SURCHARGE_ZONE_TYPES = function (err) {
    return SURCHARGE_ZONE_TYPES;
}
...
exports.update_request_status_socket = function(trip_id){
    var trip_id ="'"+trip_id+"'";
    // console.log('update_request_status_socket: '+ trip_id)
    socket_object.emit(trip_id, {
        is_trip_updated: true, trip_id: trip_id
    });
}

exports.get_polyline_path_from_google_map = function(pickup_latitude, pickup_longitude, destination_latitude, destination_longitude, return_data) {
    Settings.findOne({}, function (err, setting_detail) {

        const google_key = setting_detail.road_api_google_key;

        var request = require('request');

        const base_url = "https://maps.googleapis.com/maps/api/directions/json?";
        var url = base_url + "origin=" + pickup_latitude + "," + pickup_longitude
            + "&destination=" + destination_latitude + "," + destination_longitude 
            + "&units=metric&sensor=true&key=" + google_key;

        request(url, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                try {
                    var json_obj = JSON.parse(body);
                    if (json_obj.routes != null && json_obj.routes.length > 0) {
                        var points = json_obj.routes[0].overview_polyline.points;
                        if (points != null) {
                            return_data({success: true, polyline: points});
                        } else {
                            return_data({success: false});                            
                        }
...
exports.get_toll_cost = function(polyline, return_data) {
    ...
    Settings.findOne({}, function (err, setting_detail) {
        ...
        const options = {
            ...
            headers: {'content-type': 'application/json', 'x-api-key': setting_detail.toll_guru_key },
            body: { 
        ...
        request(options, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                try {                    
                    if (body.route.tolls != null) {
                        const toll_data = body.route.tolls;
                        const toll_data_count =  toll_data.length;
                        var toll_cost = 0;
                        for(var i = 0; i < toll_data_count; i++) {
                            toll_cost += toll_data[i].tagPriCost;
                        }
                        return_data({success: true, toll_cost: toll_cost});

                    } else {
                        return_data({success: false, message: body.message});
                    }
...
exports.get_encoded_polyline = function(point_array) {
    var polyline = require('@mapbox/polyline');
    return polyline.encode(point_array);
}

// returns an array of lat, lon pairs
exports.get_decoded_polyline = function(encoded_polyline) {
    var polyline = require('@mapbox/polyline');
    return polyline.decode(encoded_polyline);
}

// calculates surcharge amount from zones and path
exports.get_surcharge_amount = function(coordinates_array, zones) {
    const coordinates_count = coordinates_array.length;

    var surcharge_amount = 0;
    var applied_zones = [];

    if(zones.length == 0 || coordinates_count == 0) {
        return [surcharge_amount, applied_zones];
    }   

    zones.forEach(function (cityzoneDetail) {
        if(cityzoneDetail.zone_type == "None" || cityzoneDetail.amount == 0) {
            return;
        }

        if(cityzoneDetail.zone_type == "Out-of-town") {
            var in_zone_index = -1, out_zone_index = -1, i = 0;
            
            // get in-zone point first
            for(i = 0; i < coordinates_count; i++) {
                if(geolib.isPointInside(
                    {latitude: coordinates_array[i][0], longitude: coordinates_array[i][1]},
            ...
            // get out-zone point
            for(i = in_zone_index + 1; i < coordinates_count; i++) {
                if(!geolib.isPointInside(
                    {latitude: coordinates_array[i][0], longitude: coordinates_array[i][1]},
                    cityzoneDetail.kmlzone
                )) {
            ...
            applied_zones.push(cityzoneDetail.title);
            surcharge_amount += cityzoneDetail.amount;
        } else {
            // check touch only for now
            for(var i = 0; i < coordinates_count; i++) {
                if(geolib.isPointInside(
                    {latitude: coordinates_array[i][0], longitude: coordinates_array[i][1]},
                    cityzoneDetail.kmlzone
                )) {
...
exports.stripe_get_available_balance = function (account_id, currencycode, return_data) {
    ...
    stripe.balance.retrieve({
        ...
        if (error) {
            ...
            for(const obj of balance.instant_available) {
                if(obj.currency == currency) {
                    return_data({success: true, available: obj.amount});
                    return;
                }
            }

            return_data({success: false, error: error_message.ERROR_CODE_INVALID_CURRENCY_CODE_FOR_BALANCE});
        }
    });
};

exports.stripe_instant_payout = function (amount, account_id, currencycode, return_data) {
    ...
    if (amount > 0) {
        stripe.payouts.create({
            ...
            if (error) {
                ...
                return_data({success: true, payout_id: payout.id});
            }
        });
    }
};
...
exports.stripe_get_platform_balance = function (return_data) {
    var stripe_secret_key = setting_detail.stripe_secret_key;
    var stripe = require("stripe")(stripe_secret_key);
    stripe.balance.retrieve(function (error, balance) {
        if (error) {
            ...
            return_data({success: true, result: balance});
        }
    });
};