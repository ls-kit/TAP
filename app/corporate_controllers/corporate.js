    ...
    if (typeof req.session.corporate == 'undefined') {
        ...
        async.concat([Trip, Trip_history], function (model, callback) {
            ...
            function (err, result) {
                ...
                if (result.length == 0) {
                    res.render('corporate_request_list', { detail: result, 'current_page': 1, 'pages': 0, 'next': 1, 'pre': 0, moment: moment });
                } else {
                    var pages = Math.ceil(total / number_of_rec);
                    res.render('corporate_request_list', { detail: result, 'current_page': page, 'pages': pages, 'next': next, 'pre': pre, moment: moment });
                }
            })
...
exports.corporate_add_limit = function (req, res, next) {
    if (typeof req.session.corporate == 'undefined') {
        ...
        User.findOne({_id: req.body.user_id}, function(error, user){
            ...
            user.save(function(){
                res.json({success: true, message: admin_messages.success_message_add_limit});
            });
        })
    }
}