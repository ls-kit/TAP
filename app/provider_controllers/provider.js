                ...
                Country.findOne({countryname: response.country}).then((country_detail) => { 
                    ...
                    Wallet_history.aggregate([condition, referral_condition, group]).then((wallet_history_count)=>{
                        if(wallet_history_count.length>0){
                            res.render("provider_profile", {country: callingCountries.all, phone_number_min_length: country_detail.phone_number_min_length, phone_number_length: country_detail.phone_number_length, is_public_demo: is_public_demo, login1: response,total_referral_credit: wallet_history_count[0].total_referral_credit});
                            delete message;
                        } else {
                            res.render("provider_profile", {country: callingCountries.all, phone_number_min_length: country_detail.phone_number_min_length, phone_number_length: country_detail.phone_number_length, is_public_demo: is_public_demo, login1: response,total_referral_credit: 0});
                        delete message;
                        }
                    })
                });
                ...
                provider_document.save(function (err, save) {
                    Provider.findOne({ _id: req.session.provider._id }).then((provider) => {
                        Provider_Document.find({
                            ...
                            provider.save(function (err, save) {
                                res.redirect('/provider_document_panel');
                                delete message;
                            }, (err) => {
                                utils.error_response(err, res)
