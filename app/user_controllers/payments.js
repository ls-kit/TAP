            ...
            Setting.findOne({}, function (err, setting_detail) {
                if (setting_detail)
                {
                    ...
                    (err, paymentMethod)=> {
                        ...
                        Card.find({user_id: req.session.user._id, $or :  [{type: type}, { type: {$exists: false} }]}).then((card_data) => { 
                            ...
                            if (card_data.length > 0) {
                                Card.findOneAndUpdate({user_id: req.session.user._id, $or :  [{type: type}, { type: {$exists: false} }], is_default: constant_json.YES}, {is_default: constant_json.NO}).then((card_data) => { 
