        ...
        Trip.aggregate([condition, condition1, condition2, condition3, condition4, condition5, user_condition, lookup, unwind, search, filter, count]).then((array) => {
            ...
            if (array.length == 0) {
                ...
                Trip.aggregate([condition, condition1, condition2, condition3, condition4, condition5, user_condition, lookup, unwind, search, filter, sort]).then((array) => {
                    ...
                    var workbook = excelbuilder.createWorkbook('./data/xlsheet', time + '_user_future_request.xlsx');
                    

                    var sheet1 = workbook.createSheet('sheet1', 7, array.length + 1);

                    sheet1.set(1, 1, req.__('title_trip_id'));
                    sheet1.set(2, 1, req.__('title_date_time'));
                    sheet1.set(3, 1, req.__('title_pickup_address'));
                    sheet1.set(4, 1, req.__('title_destination_address'));
                    sheet1.set(5, 1, req.__('title_payment_mode'));

                    array.forEach(function (data, index) {
                        sheet1.set(1, index + 2, data.unique_id);
                        sheet1.set(2, index + 2, moment(data.server_start_time).tz(data.timezone).format("DD MMM 'YY"));
                        sheet1.set(3, index + 2, data.source_address);
                        sheet1.set(4, index + 2, data.destination_address);
                        if (data.payment_mode == 1) {
                            sheet1.set(5, index + 2, req.__('title_pay_by_cash'));
                        } else {
                            sheet1.set(5, index + 2, req.__('title_pay_by_card'));
                        }
                        ...
                        if (index == array.length - 1) {
                            workbook.save(function (err) {
                                if (err) {
                                    ...
                                    var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_user_future_request.xlsx"
                                    res.json(url);
                                    setTimeout(function () {
                                        fs.unlink('data/xlsheet/' + time + '_user_future_request.xlsx', function (err, file) {
                                        });
                                    }, 10000)
                                }
                            });
