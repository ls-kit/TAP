            ...
            Country.findOne({ countryname: response.country }).then((country_detail) => {
                ...
                Wallet_history.aggregate([condition, referral_condition, group]).then((wallet_history_count)=>{
                    if(wallet_history_count.length>0){
                        res.render("user_profile", { country: callingCountries.all, phone_number_min_length: country_detail.phone_number_min_length, phone_number_length: country_detail.phone_number_length, is_public_demo: is_public_demo, login1: response ,total_referral_credit: wallet_history_count[0].total_referral_credit});
                        delete message
                    } else {
                        res.render("user_profile", { country: callingCountries.all, phone_number_min_length: country_detail.phone_number_min_length, phone_number_length: country_detail.phone_number_length, is_public_demo: is_public_demo, login1: response ,total_referral_credit: 0});
                        delete message
                    }
                })
            });
    ...
    if (req.body.type == 1) {
        ...
        Provider.findOne({ referral_code: req.body.referral_code }).then((userData) => {
            if (!userData) {
                ...
                Country.findOne(cond).then((country) => {
                    var refered_user = country.bonus_to_providerreferral;
                    var userRefferalCount = userData.total_referrals;
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err) {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_user_history.xlsx"
                            res.json(url);
                            setTimeout(function () {
                ...
                if (index == array.length - 1) {
                    workbook.save(function (err) {
                        if (err) {
                            ...
                            var url = 'https://' + req.get('host') + "/xlsheet/" + time + "_user_schedule_trip.xlsx"
                            res.json(url);
                            setTimeout(function () {
